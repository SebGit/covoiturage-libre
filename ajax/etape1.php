...
?>
  ...
  $("#ETAPE1").keydown(function(event){
			if((event.keyCode==103)||(event.keyCode==105)||(event.keyCode==106)||(event.keyCode==108)){
				event.preventDefault();
			}
			if((event.keyCode==9)||(event.keyCode==13)){
			event.preventDefault(); 
			var villeid=$('#ville_etape1 li a:first-child').html();
			var lat=$('#ville_etape1 li a:first-child').attr('lat');
			var lon=$('#ville_etape1 li a:first-child').attr('lon');
			$('#ETAPE1').val(villeid);
			$('#ETAPE1_LAT').val(lat);
			$('#ETAPE1_LON').val(lon);
			$('#ETAPE1').removeClass('wait');
			$('#ETAPE1').addClass('check');
			$('#liste_etape1').hide();
			$('#ETAPE1').blur();/*évite certains bugs d'affichage*/
			$('#PRIX1').focus();
			}
		});
