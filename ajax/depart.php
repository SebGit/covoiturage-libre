...
*/
?>
  <a href="#" class="close" onClick="closedep()">fermer</a>
  <ul id="ville_depart">
  ...
  <script type="text/javascript">
   /*La fonction ci-dessous sert à prévenir le comportement des touches du clavier. "Tab" et "Entrée" valide le premier choix affiché. Les flèches sont quant à elles annulées (elles causaient un plantage de l'ajax)*/
  		$("#DEPART").keydown(function(event){
			if((event.keyCode==103)||(event.keyCode==105)||(event.keyCode==106)||(event.keyCode==108)){
				event.preventDefault();
			}
			if((event.keyCode==9)||(event.keyCode==13)){
			event.preventDefault(); 
			var villeid=$('#ville_depart li a:first-child').html();
			var lat=$('#ville_depart li a:first-child').attr('lat');
			var lon=$('#ville_depart li a:first-child').attr('lon');
			$('#DEPART').val(villeid);
			$('#DEPART_LAT').val(lat);
			$('#DEPART_LON').val(lon);
			$('#DEPART').removeClass('wait');
			$('#DEPART').addClass('check');
			$('#liste_depart').hide();
			$('#DEPART').blur();/*évite certains bugs d'affichage*/
			$('#villeetape').html(villeid);
			}
		});
