...
<?php if($totalRows_RSville>0){ do { ?>
  ...
  $("#ARRIVEE").keydown(function(event){
			if((event.keyCode==103)||(event.keyCode==105)||(event.keyCode==106)||(event.keyCode==108)){
				event.preventDefault();
			}
			if((event.keyCode==9)||(event.keyCode==13)){
			event.preventDefault(); 
			var villeid=$('#ville_arrivee li a:first-child').html();
			var lat=$('#ville_arrivee li a:first-child').attr('lat');
			var lon=$('#ville_arrivee li a:first-child').attr('lon');
			$('#ARRIVEE').val(villeid);
			$('#ARRIVEE_LAT').val(lat);
			$('#ARRIVEE_LON').val(lon);
			$('#ARRIVEE').removeClass('wait');
			$('#ARRIVEE').addClass('check');
			$('#liste_arrivee').hide();
			$('#ARRIVEE').blur();/*évite certains bugs d'affichage*/
			}
		});
